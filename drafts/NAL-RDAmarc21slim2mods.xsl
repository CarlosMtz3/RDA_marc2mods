<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	exclude-result-prefixes="f marc marccountry nalsubcat saxon xd xlink xs">
...
Revision 1.02 - Added Log Comment  2003/03/24 19:37:42  ckeith
	...
	<xsl:template match="/">
		...
		<xsl:call-template name="unprefixed-marc">
			<xsl:with-param name="in-xml">
				<xsl:copy-of select="//record[descendant-or-self::node()]"/>
			</xsl:with-param>
		</xsl:call-template>
	...
	<xsl:template name="unprefixed-marc">
		<xsl:param name="in-xml" select="/collection[record][descendant::node()]"/>
    ...
    -->
			...
			<xsl:choose>
				...
				<xsl:when
					...
					<xsl:if test="contains(., $controlField008-11-14)">
		...
		<xsl:for-each select="datafield[@tag = 264][@ind2 = 1]">
			...
			<!--		<xsl:if test="count(following-sibling::subfield[@code = 'b'])!=0">-->
						<xsl:for-each select="subfield[@code = 'b']">
							<xsl:value-of select="substring-before(subfield[@code = 'b'][current()], ',')"/>
						</xsl:for-each>
					<!--</xsl:if>-->
		...
		<xsl:variable name="physicalDescription">
			...
			<xsl:for-each
				...
				<internetMediaType>
					<xsl:value-of select="."/>
				</internetMediaType> `1./ </xsl:for-each>
...
-->
		...
		<xsl:for-each select="datafield[@tag = 072][@ind2 = '0']">
			...
			<subject authority="agricola">
				<topic type="code" authority="NAL subject category codes">
					<xsl:value-of select="$code"/>
				</topic>
				<topic type="text" authority="NAL subject category codes">
		...
		<recordInfo>
			...
			<recordOrigin>
				...
				<xsl:value-of
					select="normalize-space(concat('Converted from MARCXML to MODS version 3.4 using', ' ', $transform, ' ', '(Revision 2.00 2022/10/27)'))"
				/>
			</recordOrigin>
	...
	<xsl:template name="displayForm">
		<xsl:for-each select="subfield[@code = 'a']">
			...
			<!--when only a creators initials are present DO NOT remove punctuation from the end-->
			<namePart type="given">
				<xsl:choose>
...
-->
	...
	<xd:doc>dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd <xd:desc> tmee
			&lt;xsl:template name="createNoteFrom534"&gt; &lt;note type="original version"&gt;
			&lt;xsl:call-template name="xxx880"/&gt; &lt;xsl:call-template name="uri"/&gt;
			&lt;xsl:variable name="str"&gt; &lt;xsl:for-each select="subfield[@code!='6' and
			@code!='8']"&gt; &lt;xsl:value-of select="."/&gt; &lt;xsl:text&gt; &lt;/xsl:text&gt;
			&lt;/xsl:for-each&gt; &lt;/xsl:variable&gt; &lt;xsl:value-of
			select="substring($str,1,string-length($str)-1)"/&gt; &lt;/note&gt;
	...
	<xd:doc scope="component">
		<xd:desc>
			...
			<xd:p><xd:b>Usage: </xd:b>f:change-element-ns-deep([xpath/nodes],[xpath/newns],
				[xpath/prefix])</xd:p>
	...
	<xsl:function name="f:change-element-ns" as="element()?">
		...
		<xsl:for-each select="$elements">
			...
			<xsl:element name="{concat($prefix,
				...
				local-name($element))}" namespace="{$newns}">
	...
	<xsl:function name="f:subjCatCode" as="xs:string">
		...
		<xsl:sequence
			select="$nodes/nalsubcat:NALSubjectCategoryCodes/nalsubcat:value[nalsubcat:code = $code]/nalsubcat:category"
		/>
	</xsl:function>
	...
	<xsl:function name="f:convertMARCCountry" as="xs:string">
		...
		<xsl:variable name="nodes">
			<xsl:copy-of select="document('./marcCountry.xml')"/>
		</xsl:variable>
		<xsl:value-of
	...
	<xd:doc>
		<xd:desc>
			...
			<xd:p><xd:b>Purpose:</xd:b>The f:format-case function treats a string like two parts;
	...
	<xsl:function name="f:formatNames">
		...
		<xsl:sequence select="
				if (matches($arg, $white-space) and (contains($arg, $white-space)))
				then
					concat(f:format-case(substring-before($arg, $white-space)), ($white-space), (f:format-case(substring-after($arg, $white-space))))
				else
					$arg"/>
		<!--</xsl:when>
			<xsl:otherwise>
