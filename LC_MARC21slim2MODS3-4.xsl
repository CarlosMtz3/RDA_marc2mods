<xsl:stylesheet exclude-result-prefixes="f marc marccountry nalsubcat saxon xd xlink xs xsi" version="2.0" xmlns="http://www.loc.gov/mods/v3" xmlns:f="http://functions" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:marccountry="http://www.local.gov/marc/countries" xmlns:nalsubcat="http://nal-subject-category-codes" xmlns:saxon="http://saxon.sf.net/" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
...
Revision 1.02 - Added Log Comment  2003/03/24 19:37:42  ckeith
	...
	<xsl:template match="/">
		<xsl:choose>
			...
			<xsl:otherwise>
				<mods version="3.4" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-4.xsd">
...
-->
		...
		<xsl:for-each select="datafield[@tag = '880']">
			<xsl:apply-templates mode="trans880" select="self::*"/>
		</xsl:for-each>
		...
		<xsl:for-each select="datafield[@tag = '020']">
			<xsl:if test="subfield[@code = 'z']">
				<identifier invalid="yes" type="isbn">
		...
		<xsl:for-each select="datafield[@tag = '022'][subfield[@code = 'z']]">
			<xsl:if test="subfield[@code = 'z']">
				<identifier invalid="yes" type="issn">
					<xsl:value-of select="subfield[@code = 'z']"/>
				</identifier>
			</xsl:if>
		</xsl:for-each>
		...
		<xsl:for-each select="datafield[@tag = '022'][subfield[@code = 'm']]">
			<xsl:if test="subfield[@code = 'm']">
				<identifier invalid="yes" type="issn-l">
		...
		<xsl:for-each select="datafield[@tag = '010'][subfield[@code = 'z']]">
			<identifier invalid="yes" type="lccn">
		...
		<xsl:for-each name="displayForm" select="datafield[@tag = '037']">
...
-->
	...
	<xsl:template name="createNoteFrom500">
		<note>
			<xsl:call-template name="xxx880"/>
			<xsl:call-template name="uri"/>
5			<xsl:value-of select="subfield[@code = 'a']"/>
		</note>
	</xsl:template>
...
<!-- tmee 1.93 20140130 -->
	...
	<xsl:template match="* | @*" mode="global_copy">
		<xsl:copy>
			<xsl:apply-templates mode="global_copy" select="* | @* | text()"/>
		</xsl:copy>
	</xsl:template>
